...
def main() -> None:
    ...
    compression = fl.CompressionFactory.build(
        executor_factory=tff.CompressionExecutorFactory(),
        sparsity=0.1,
        client_weighting=tff.learning.ClientWeighting.NUM_EXAMPLES,
        server_optimizer_fn=lambda: tf.keras.optimizers.SGD(learning_rate=1.0),
    )

    # Create strategy with gradient compression
    strategy = fl.server.strategy.FedAvg(
        ...
        initial_parameters=fl.common.ndarrays_to_parameters(
            model.get_weights()),
        compression=compression,  # Include compression algorithm
    )
